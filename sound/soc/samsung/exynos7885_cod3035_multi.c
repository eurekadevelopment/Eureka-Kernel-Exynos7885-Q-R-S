		...
		int index, int event)
{
	...
	dev_err(card->dev, "%s called: index: %d, event: %d\n",
		__func__, index, event);

	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		if (gpio_is_valid(drvdata->dmic_path_en)) {
			gpio_set_value(drvdata->dmic_path_en, 1);
			dev_info(card->dev, "dmic_path_en On\n");
		}
		break;

	case SND_SOC_DAPM_POST_PMD:
		if (gpio_is_valid(drvdata->dmic_path_en)) {
			gpio_set_value(drvdata->dmic_path_en, 0);
			dev_info(card->dev, "dmic_path_en Off\n");
		}
		break;
	default:
		break;
...
static void universal7885_dmic_path_parse_dt(struct platform_device *pdev)
{
	struct device_node *np = pdev->dev.of_node;
	struct snd_soc_card *card = platform_get_drvdata(pdev);
	struct exynos7885_drvdata *drvdata = card->drvdata;
	...
	gpio = of_get_named_gpio(np, "dmic-path-en", gpio_cnt++);
	if (gpio_is_valid(gpio))
		drvdata->dmic_path_en = gpio;
	else{
		dev_err(&pdev->dev, "Invalid dmic_path_en gpio\n");
		return;
	}

	dev_info(card->dev, "Success to get dmic_path_en gpio (%d)\n", gpio);
}
...
static int exynos7885_audio_probe(struct platform_device *pdev)
{
	...
	for_each_child_of_node(np, dai) {
		...
		if (!exynos7885_dai[nlink].codec_name) {
			struct snd_soc_dai_link *link = &exynos7885_dai[nlink];
