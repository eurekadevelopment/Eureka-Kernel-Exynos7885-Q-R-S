...
void pmucal_smc_read(struct pmucal_seq *seq, int update)
{
	...
	if (seq->offset == PMU_CP_CTRL_S_OFFSET) {
		...
		pr_err("%s%s: ERR! read Fail: seq->offset(%x) %d\n", PMUCAL_PREFIX, __func__, seq->offset, cp_ctrl & 0xffff);
		return;
	}
...
void pmucal_smc_write(struct pmucal_seq *seq)
{
	...
	if (seq->offset == PMU_CP_CTRL_S_OFFSET) {
		...
		pr_err("%s%s: ERR! CP_CTRL Write Fail: seq->offset(%x) %d\n", PMUCAL_PREFIX, __func__,seq->offset, ret);
		return;
	}
