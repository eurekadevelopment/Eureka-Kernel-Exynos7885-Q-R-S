/* linux/drivers/devfreq/exynos/exynos7872_bus_int.c
 *
 * Copyright (c) 2015 Samsung Electronics Co., Ltd.
 *              http://www.samsung.com
 *
 * Samsung EXYNOS7872 SoC INT devfreq driver
 *
...
static int exynos7872_devfreq_int_cmu_dump(struct exynos_devfreq_data *data)
{
...
static int exynos7872_devfreq_int_reboot(struct exynos_devfreq_data *data)
{
...
static int exynos7872_devfreq_int_get_freq(struct device *dev, u32 *cur_freq,
...
static int exynos7872_devfreq_int_set_freq(struct device *dev, u32 new_freq,
		struct clk *clk, struct exynos_devfreq_data *data)
{
...
static int exynos7872_devfreq_int_pm_suspend_prepare(struct exynos_devfreq_data *data)
{
...
static int exynos7872_devfreq_int_resume(struct exynos_devfreq_data *data)
{
	u32 cur_freq;

	/* for sync from resume frequency */
	if (exynos7872_devfreq_int_get_freq(data->dev, &cur_freq, data->clk, data)) {
...
static int exynos7872_devfreq_int_init_freq_table(struct exynos_devfreq_data *data)
{
...
static int exynos7872_devfreq_int_get_status(struct exynos_devfreq_data *data)
{
...
static int __init exynos7872_devfreq_int_init_prepare(struct exynos_devfreq_data *data)
{
	data->ops.get_dev_status = exynos7872_devfreq_int_get_status;
	data->ops.get_freq = exynos7872_devfreq_int_get_freq;
	data->ops.set_freq = exynos7872_devfreq_int_set_freq;
	data->ops.init_freq_table = exynos7872_devfreq_int_init_freq_table;
	data->ops.pm_suspend_prepare = exynos7872_devfreq_int_pm_suspend_prepare;
	data->ops.resume = exynos7872_devfreq_int_resume;
	data->ops.reboot = exynos7872_devfreq_int_reboot;
	data->ops.cmu_dump = exynos7872_devfreq_int_cmu_dump;

	return 0;
}

static int __init exynos7872_devfreq_int_initcall(void)
{
	if (register_exynos_devfreq_init_prepare(DEVFREQ_INT,
				exynos7872_devfreq_int_init_prepare))
		return -EINVAL;

	return 0;
}

fs_initcall(exynos7872_devfreq_int_initcall);
