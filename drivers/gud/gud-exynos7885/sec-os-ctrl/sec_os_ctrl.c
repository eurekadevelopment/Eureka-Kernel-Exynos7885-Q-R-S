		...
		struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	if (buf[0] == 'L') {
		...
		} else if (core_num < 6) {	/* From core 0 to core 5 */
			new_core = core_num;
		} else {
			pr_err("[LITTLE] Enter correct core number(0~5)\n");
			return count;
		}
	} else if (buf[0] == 'b') {
		if ((buf[1] == 0xA) || (buf[1] == 0x0)) {	/* if LF(Line Feed, 0xA) or NULL(0x0) */
			...
			pr_err("[big] Enter correct core number(0~1)\n");
			return count;
		}
	}
		...
		struct kobj_attribute *attr, char *buf)
{
	...
	return sprintf(buf, "Secure OS is on core [%c%d]\n",
			(current_core < 6) ? 'L' : 'b', (current_core < 6) ? current_core : (current_core - 6));
}
