	...
	if (operation) {
		...
		mc_dev_devel("	cycle %d, TEEC_TEMP_IN* - zero pointer or size",
			...
			mc_dev_devel("	cycle %d, TEEC_MEMREF_PARTIAL_*", i);
				...
				break;
			default:
				mc_dev_devel("cycle %d, default", i);
				*return_origin = TEEC_ORIGIN_API;
		...
		if ((teec_result == TEEC_SUCCESS) && (n_buf > 0)) {
			...
			if (!ret) {
				...
				mc_dev_devel("client map failed: %d", ret);
				*return_origin = TEEC_ORIGIN_COMMS;
				  ...
				  u32 *return_origin)
{
	...
	for (i = 0; i < _TEEC_PARAMETER_NUMBER; i++) {
		...
		default:
			mc_dev_devel("cycle %d, bad parameter", i);
		}
	}
	...
	if (n_buf > 0) {
		...
		ret = client_unmap_session_wsms(session->context.client,
						...
						map.bufs);
		if (ret < 0)
			mc_dev_devel("client unmap failed: %d", ret);
	}
...
static void _teec_close_session(struct teec_session_imp *session_imp)
{
	...
	if (ret)
		mc_dev_devel("%s failed: %d", __func__, ret);
			 ...
			 u32 *return_origin)
{
	...
	if (teec_res != TEEC_SUCCESS) {
		mc_dev_devel("_teec_setup_operation failed (%08x)", teec_res);
		return teec_res;
	}
	...
	if (teec_res != TEEC_SUCCESS)
		...
		mc_dev_devel("_teec_unwind_operation (%08x)", teec_res);
		      ...
		      u32 *return_origin)
{
	...
	if (teec_res != TEEC_SUCCESS) {
		mc_dev_devel("_teec_call_ta failed(%08x)", teec_res);
		/*
	...
	if (teec_res != TEEC_SUCCESS) {
		mc_dev_devel("TA OpenSession EP failed(%08x)", teec_res);
		goto error;
	}
