...
{
	nq_session_init(&session->nq_session, is_gp);
			  ...
			  bool silent_expiry)
{
	...
	if (!ret)
		nq_session_state_update(&session->nq_session, false);
	else if (ret != -ERESTARTSYS)
		nq_session_state_update(&session->nq_session, true);
	...
	if (ret && ((ret != -ETIME) || !silent_expiry)) {
		...
		if (ret == -ERESTARTSYS && system_freezing_cnt.counter == 1)
			mc_dev_devel("freezing session %x\n",
				     session->nq_session.id);
		else
#endif
			mc_dev_devel("session %x ec %d ret %d\n",
				     session->nq_session.id, session->exit_code,
				     ret);
	}
		     ...
		     const struct mcp_buffer_map *tci_map)
{
	...
	if (!ret) {
		nq_session_set_id(&session->nq_session,
				  cmd.rsp_open.session_id);
		...
		if (l_ctx.unexp_notif.session_id == session->nq_session.id) {
			mutex_lock(&session->exit_code_lock);
...
int mcp_close_session(struct mcp_session *session)
{
	...
	cmd.cmd_close.session_id = session->nq_session.id;
	...
	mc_dev_devel("close session %x ret %d state %d\n",
		     session->nq_session.id, ret, session->state);
	return ret;
}
...
{
	if (session->nq_session.id == SID_MCP)
		mc_dev_devel("notify MCP\n");
	else
		mc_dev_devel("notify session %x\n", session->nq_session.id);

	return nq_session_notify(&session->nq_session, 0);
}
					 ...
					 u32 exit_code)
{
	...
	mc_dev_devel("notification from session %x exit code %d state %d\n",
		     id, exit_code, session ? session->state : -1);
	if (session) {
		/* TA has terminated */
		if (exit_code) {
	...
	if (!session) {
		...
		l_ctx.unexp_notif.payload = exit_code;
		mutex_unlock(&l_ctx.unexp_notif_mutex);
	}
}

static void mcp_notif_handler(struct nq_session *session, u32 id, u32 payload)
{
	if (id == SID_MCP) {
		/* MCP notification */
		...
		mcp_session = container_of(session, struct mcp_session,
					   nq_session);
		session_notif_handler(mcp_session, id, payload);
	}
}
...
int mcp_init(void)
{
	...
	mcp_session_init(&l_ctx.mcp_session, false, NULL);
	l_ctx.mcp_session.nq_session.id = SID_MCP;
	...
	nq_register_notif_handler(mcp_notif_handler, true);
	return 0;
}
