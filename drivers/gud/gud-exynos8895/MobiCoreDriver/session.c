...
static void session_close_worker(struct work_struct *work)
{
	...
	mc_dev_devel("session %x worker", session->nq_session.id);
		      ...
		      struct mc_ioctl_buffer *buf)
{
	...
	wsm->mmu = client_mmu_create(session->client, session->task,
			       ...
			       struct mc_ioctl_buffer *buf)
{
	...
	mmu = client_mmu_create(session->client, session->task, session->flags,
 ...
 */
static struct task_struct *fd_to_task(int client_fd)
{
	struct socket *sock;
	struct task_struct *task;
	int err;
				   ...
				   int client_fd, u32 flags)
{
	...
	/* Only the Android proxy can provide a PID */
	if (client_fd >= 0) {
#ifdef CONFIG_ANDROID
		task = fd_to_task(client_fd);
		if (!task)
	...
	if (is_gp) {
		...
		int ret = check_prepare_identity(identity, &mcp_identity, task);
...
int session_close(struct tee_session *session)
{
	...
	mc_dev_devel("closed session %x", session->nq_session.id);
...
static void session_release(struct kref *kref)
{
	...
	mc_dev_devel("freed session %p: client %p id %x\n",
		     session, session->client, session->nq_session.id);
			 ...
			 struct mcp_buffer_map *new_maps, bool use_multimap)
{
	...
	if (to_unmap) {
		...
		ret = mcp_multiunmap(session->nq_session.id, maps,
				     use_multimap);
		if (ret)
			return ret;
	...
	/* Map buffers */
	return mcp_multimap(session->nq_session.id, new_maps, use_multimap);
}
			  ...
			  struct tee_session *session, bool is_closing)
{
	...
	ret = kasnprintf(buf, "\tsession %p [%d]: %x %s PID %d ec %d%s\n",
			 ...
			 session->nq_session.id,
			 nq_session_is_gp(&session->mcp_session.nq_session) ?
			 "GP" : "MC", session->task->tgid, exit_code,
			 is_closing ? " <closing>" : "");
	if (ret < 0)
		return ret;
