...
static struct {
	...
	void (*mc_notif_handler)(struct nq_session *s, u32 id, u32 payload);
	void (*gp_notif_handler)(struct nq_session *s, u32 id, u32 payload);
...
static inline bool nq_notifications_flush_nolock(void)
{
	...
	while (!list_empty(&l_ctx.notifications) && !notif_queue_full()) {
		...
		session = list_first_entry(&l_ctx.notifications,
					   ...
					   notifications_list);
		mc_dev_devel("pop %x\n", session->id);
		notif_queue_push(session->id, session->notification_payload);
		session->notif_state = NQ_NOTIF_SENT;
...
int nq_session_notify(struct nq_session *session, u32 payload)
{
	...
	if (!list_empty(&l_ctx.notifications) || notif_queue_full()) {
		if (!list_empty(&session->notifications_list)) {
			if (payload != session->notification_payload) {
				mc_dev_err("skip %x payload change %u -> %u",
					   session->id,
					   session->notification_payload,
					   payload);
			} else {
				mc_dev_devel("skip %x payload %u\n",
					     session->id, payload);
			}
			ret = -EAGAIN;
		} else {
			mc_dev_devel("push %x payload %u\n", session->id,
				     payload);
			session->notification_payload = payload;
			list_add_tail(&session->notifications_list,
				      &l_ctx.notifications);
			session->notif_state = NQ_NOTIF_QUEUED;
			session->notif_cpu_clk = local_clock();
		}
...
void nq_update_time(void)
{
	...
	getnstimeofday(&tm);
	l_ctx.time->seconds = tm.tv_sec;
	l_ctx.time->nsec = tm.tv_nsec;
}
...
{
	if (gp)
		l_ctx.gp_notif_handler = handler;
	else
		l_ctx.mc_notif_handler = handler;
}

static inline void nq_notif_handler(u32 session_id, u32 payload)
{
	struct nq_session *session = NULL, *s;

	mutex_lock(&l_ctx.sessions_mutex);
	mc_dev_devel("notification from session %x payload %d\n",
		     session_id, payload);
	list_for_each_entry(s, &l_ctx.sessions, list) {
		if (s->id == session_id) {
			session = s;
			break;
		}
	...
	if (session) {
		/* Unblock waiter */
		mutex_lock(&l_ctx.notifications_mutex);
		session->notif_state = NQ_NOTIF_RECEIVED;
		session->notif_cpu_clk = local_clock();
		mutex_unlock(&l_ctx.notifications_mutex);
		if (session->is_gp)
			l_ctx.gp_notif_handler(session, session_id, payload);
		else
			l_ctx.mc_notif_handler(session, session_id, payload);
	} else {
		l_ctx.mc_notif_handler(NULL, session_id, payload);
	}
	mutex_unlock(&l_ctx.sessions_mutex);
}
...
static int irq_bh_worker(void *arg)
{
	...
	while (l_ctx.irq_bh_active) {
		wait_for_completion(&l_ctx.irq_bh_complete);
...
void nq_session_init(struct nq_session *session, bool is_gp)
{
	...
	session->notif_state = NQ_NOTIF_IDLE;
	session->notif_cpu_clk = 0;
}
...
void nq_session_state_update(struct nq_session *session, bool dead)
{
	mutex_lock(&l_ctx.notifications_mutex);
	if (dead)
		session->notif_state = NQ_NOTIF_DEAD;
	else
		session->notif_state = NQ_NOTIF_CONSUMED;
	session->notif_cpu_clk = local_clock();
	mutex_unlock(&l_ctx.notifications_mutex);
}
...
int nq_init(void)
{
	...
	mutex_init(&l_ctx.sessions_mutex);

	/* NQ_NUM_ELEMS must be power of 2 */
	q_len = ALIGN(2 * (sizeof(struct notification_queue_header) +
			   NQ_NUM_ELEMS * sizeof(struct notification)), 4);
	buf_len = q_len + sizeof(*l_ctx.time) + sizeof(*l_ctx.mcp_buffer);
	if (buf_len > (u16)-1) {
