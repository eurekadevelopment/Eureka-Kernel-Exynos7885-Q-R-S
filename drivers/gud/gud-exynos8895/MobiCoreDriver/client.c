...
int client_open_session(struct tee_client *client, u32 *session_id,
			...
			struct mc_identity *identity, int client_fd, u32 flags)
{
	int err = 0;
	u32 sid = 0;
		       ...
		       struct mc_identity *identity, int client_fd, u32 flags)
{
	 ...
	 */
	session = session_create(client, is_gp, identity, client_fd, flags);
	if (IS_ERR(session))
	...
	*session_id = session->nq_session.id;
	mutex_unlock(&client->sessions_lock);
...
int client_remove_session(struct tee_client *client, u32 session_id)
{
	...
	list_for_each_entry(candidate, &client->sessions, list) {
		if (candidate->nq_session.id == session_id) {
			session = candidate;
			list_move(&session->list, &client->closing_sessions);
			break;
		}
...
struct tee_mmu *client_mmu_create(struct tee_client *client,
				  struct task_struct *task, u32 flags,
				  uintptr_t va, u32 len, struct cbuf **cbuf_p)
{
	...
	*cbuf_p = cbuf;
	if (cbuf) {
		...
		/* Cannot map foreign app buffers if flag not set */
		if (!(flags & MC_IO_SESSION_REMOTE_BUFFERS))
			task = current;
	}
