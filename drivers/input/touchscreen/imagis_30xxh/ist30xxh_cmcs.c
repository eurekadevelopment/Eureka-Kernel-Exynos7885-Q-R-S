        ...
        u32 flag)
{
    ...
    if (flag & CMCS_FLAG_CMJIT) {
        ...
        ret = ist30xx_item_test(data, ts_cmcs->items, ts_cmcs->param,
                CMCS_CMJIT, CMCS_FLAG_CMJIT, buf32, ts_cmcs_buf->cm_jit);
        cmcs_next_step(ret);
    }
                 ...
                 struct device_attribute *attr, char *buf)
{
    ...
    mutex_lock(&data->lock);
    ret = ist30xx_cmcs_test(data, ts_cmcs_bin, ts_cmcs_bin_size, CMCS_FLAG_ALL);
    mutex_unlock(&data->lock);
...
static DEVICE_ATTR(cmcs_hvdd_custom, S_IRUGO, ist30xx_cmcs_hvdd_custom_show, NULL);
