	...
	if ((buf[1] + buf[2]) != 0xFF) {
		...
		if(retry_count < 3) {
			...
			NVT_ERR("Set default fw_ver=%d, x_num=%d, y_num=%d, "
					...
					ts->fw_ver_ic, ts->x_num, ts->y_num,
...
static void nvt_parse_dt(struct device *dev)
{
	...
	if(of_property_read_u32(np, "novatek,factory_item_version", &ts->item_version) < 0)
		ts->item_version = 0;
	NVT_LOG("novatek,item_version=%d\n", ts->item_version);
...
/*
	//--- dump I2C buf ---
	for (i = 0; i < 10; i++) {
		printk("NVT i2c buf: %02X %02X %02X %02X %02X %02X  ", point_data[1+i*6], point_data[2+i*6], point_data[3+i*6], point_data[4+i*6], point_data[5+i*6], point_data[6+i*6]);
	}
	printk("\n");
*/
...
static void get_fw_ver_ic(void *device_data)
{
	...
	CTP_I2C_READ(ts->client, I2C_FW_Address, data, 2);
...
static void get_chip_name(void *device_data)
{
	...
	snprintf(buff, sizeof(buff), "%s", "NT36672");
	sec_cmd_set_cmd_result(sec, buff, strnlen(buff, sizeof(buff)));
	sec->cmd_state = SEC_CMD_STATUS_OK;
...
static void get_checksum_data(void *device_data)
{
	...
	for (i = 0; i < BLOCK_64KB_NUM; i++) {
		...
		snprintf(buff + (i * 4), 5, "%04X", ts->csum_result[i]);
	}
...
static void factory_cmd_result_all(void *device_data)
{
	...
	snprintf(buff, sizeof(buff), "%d", ts->item_version);
