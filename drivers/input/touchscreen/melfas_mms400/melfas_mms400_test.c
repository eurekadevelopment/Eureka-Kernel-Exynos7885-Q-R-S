		...
		u8 row_num, u8 col_num, u8 buf_col_num, u8 rotate, u8 key_num)
{
	...
	print_raw = kzalloc(sizeof(u8) * (max_x + 1), GFP_KERNEL);
	memset(print_raw, 0, sizeof(u8) * (max_x + 1));
...
int mms_run_test(struct mms_ts_info *info, u8 test_type)
{
	...
	case MIP_TEST_TYPE_CM_DELTA:
		...
		if (info->tsp_dump_lock == 1)
			input_raw_info(true, &info->client->dev, "=== Cm Delta Test ===\n");
	...
	case MIP_TEST_TYPE_OPEN:
		...
		if (info->tsp_dump_lock == 1)
			input_raw_info(true, &info->client->dev, "=== OPEN Test(TSP Connection check) ===\n");
