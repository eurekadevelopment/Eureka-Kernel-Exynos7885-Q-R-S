...
ssize_t get_magnetic_ak09918c_adc(struct ssp_data *data, char *buf)
{
	...
	if (!(atomic64_read(&data->aSensorEnable) & (1ULL <<
...
ssize_t get_magnetic_ak09918c_dac(struct ssp_data *data, char *buf)
{
	...
	if (!data->uMagCntlRegData) {
		...
		data->uMagCntlRegData = buffer[21];
...
void magnetic_ak09918c_function_pointer_initialize(struct ssp_data *data)
{
	data->magnetic_ops = &magnetic_ak09918c_ops;
}
