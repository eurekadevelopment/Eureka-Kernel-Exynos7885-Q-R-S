...
ssize_t get_barometer_bmp280_name(char *buf)
{
	return sprintf(buf, "%s\n", "BMP280");
}

ssize_t get_barometer_bmp280_vendor(char *buf)
{
	return sprintf(buf, "%s\n", "BOSCH");
}


ssize_t set_barometer_bmp280_sea_level_pressure(struct ssp_data *data,
                                                const char *buf)
{
	int ret = 0;
...
ssize_t get_barometer_bmp280_calibration(struct ssp_data *data, char *buf)
{
...
ssize_t set_barometer_bmp280_calibration(struct ssp_data *data, const char *buf)
{
	int pressure_cal = 0;
	int ret = 0;
...
ssize_t get_barometer_bmp280_eeprom_check(struct ssp_data *data, char *buf)
{
	char *buffer = NULL;
	int buffer_length = 0;
	int ret = 0;
...
ssize_t get_barometer_bmp280_temperature(struct ssp_data *data, char *buf)
{
	s32 temperature = 0;
...
struct barometer_sensor_operations barometer_bmp280_ops = {
	.get_barometer_name = get_barometer_bmp280_name,
	.get_barometer_vendor = get_barometer_bmp280_vendor,
	.get_barometer_eeprom_check = get_barometer_bmp280_eeprom_check,
	.get_barometer_calibration = get_barometer_bmp280_calibration,
	.set_barometer_calibration = set_barometer_bmp280_calibration,
	.set_barometer_sea_level_pressure = set_barometer_bmp280_sea_level_pressure,
	.get_barometer_temperature = get_barometer_bmp280_temperature,
};

void barometer_bmp280_function_pointer_initialize(struct ssp_data *data)
{
	data->barometer_ops = &barometer_bmp280_ops;
}
