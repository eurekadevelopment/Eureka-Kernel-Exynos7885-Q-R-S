...
static int fw_write_unprotect_stm(struct spi_device *spi)
{
	struct stm32fwu_spi_cmd cmd;
	struct stm32fwu_spi_cmd dummy_cmd;
	int ret;
	...
	ssp_dbgf();
	cmd.cmd = WR_UNPROT_COMMAND;
	cmd.xor_cmd = XOR_WR_UNPROT_COMMAND;
	cmd.timeout = DEF_ACKCMD_NUMBER;
	...
	if (ret != BL_ACK) {
		ssp_err("fw_write_unprotect failed - %d", ret);
		return ret;
	}
...
static int fw_readout_unprotect_stm(struct spi_device *spi)
{
	struct stm32fwu_spi_cmd cmd;
	struct stm32fwu_spi_cmd dummy_cmd;
	int ret;
	...
	ssp_dbgf();
	cmd.cmd = RO_UNPROT_COMMAND;
	cmd.xor_cmd = XOR_RO_UNPROT_COMMAND;
	cmd.timeout = DEF_ACKCMD_NUMBER;
	cmd.ack_pad = BL_DUMMY;
	...
	if (ret != BL_ACK) {
		ssp_err("fw_readout_unprotect failed - %d", ret);
		return ret;
	}
...
static int update_mcu_bin(struct ssp_data *data, int iBinType)
{
	...
	int ret = SUCCESS;
	struct stm32fwu_spi_cmd cmd;
        u64 current_ts = 0, elapsed = 0;

        int readout_protect = 1, write_protect = 1;
...
update_start:
	...
	if (ret != BL_ACK) {
        ...
        if (readout_protect-- > 0) {
            ...
            ret = fw_readout_unprotect_stm(data->spi);
            elapsed = get_current_timestamp() - current_ts;
            ssp_info("fw_unprotect_readout_stm %d %lld", ret, elapsed);
            if (ret < 0) {
                ssp_errf("fw_readout_unprotect_stm %d", ret);
            } else {
        ...
        if (write_protect-- > 0) {
            current_ts = get_current_timestamp();
            ret = fw_write_unprotect_stm(data->spi);
            elapsed = get_current_timestamp() - current_ts;
            ssp_info("fw_unprotect_write_stm %d %lld", ret, elapsed);
            if (ret < 0) {
                ssp_errf("fw_write_unprotect_stm %d", ret);
            } else {
