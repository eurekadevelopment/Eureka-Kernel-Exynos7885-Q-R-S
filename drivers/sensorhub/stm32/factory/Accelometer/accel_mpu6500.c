...
ssize_t get_accel_mpu6500_name(char *buf)
{
	return sprintf(buf, "%s\n", "MPU6500");
}

ssize_t get_accel_mpu6500_vendor(char *buf)
{
	return sprintf(buf, "%s\n", "INVENSENSE");
}
...
ssize_t get_accel_mpu6500_calibration(struct ssp_data *data, char *buf)
{
	int ret = 0;
...
ssize_t set_accel_mpu6500_calibration(struct ssp_data *data, const char *buf)
{
	int ret = 0;
	int64_t dEnable = 0;
...
ssize_t get_accel_mpu6500_raw_data(struct ssp_data *data, char *buf)
{
...
ssize_t get_accel_mpu6500_reactive_alert(struct ssp_data *data, char *buf)
{
	bool bSuccess = false;
...
ssize_t set_accel_mpu6500_reactive_alert(struct ssp_data *data, const char *buf)
{
	int ret = 0;
	char chTempBuf = 1;
	struct ssp_msg *msg;
...
ssize_t get_accel_mpu6500_selftest(struct ssp_data *data, char *buf)
{
...
ssize_t set_accel_mpu6500_lowpassfilter(struct ssp_data *data, const char *buf)
{
	int ret = 0;
	int new_enable = 1;
...
struct accelometer_sensor_operations accel_mpu6500_ops = {
	.get_accel_name = get_accel_mpu6500_name,
	.get_accel_vendor = get_accel_mpu6500_vendor,
	.get_accel_calibration = get_accel_mpu6500_calibration,
	.set_accel_calibration = set_accel_mpu6500_calibration,
	.get_accel_raw_data = get_accel_mpu6500_raw_data,
	.get_accel_reactive_alert = get_accel_mpu6500_reactive_alert,
	.set_accel_reactive_alert = set_accel_mpu6500_reactive_alert,
	.get_accel_selftest = get_accel_mpu6500_selftest,
	.set_accel_lowpassfilter = set_accel_mpu6500_lowpassfilter,
};

void accelometer_mpu6500_function_pointer_initialize(struct ssp_data *data)
{
	data->accel_ops = &accel_mpu6500_ops;
}