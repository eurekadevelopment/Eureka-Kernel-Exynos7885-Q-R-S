...
ssize_t set_barometer_lps25h_sea_level_pressure(struct ssp_data *data, const char *buf)
{
	int ret = 0;
...
ssize_t get_barometer_lps25h_eeprom_check(struct ssp_data *data, char *buf)
{
	char chTempBuf  = 0;
	int ret = 0;
...
struct barometer_sensor_operations barometer_lps25h_ops = {
	...
	.get_barometer_eeprom_check = get_barometer_lps25h_eeprom_check,
...
void barometer_lps25h_function_pointer_initialize(struct ssp_data *data)
{
	data->barometer_ops = &barometer_lps25h_ops;
}