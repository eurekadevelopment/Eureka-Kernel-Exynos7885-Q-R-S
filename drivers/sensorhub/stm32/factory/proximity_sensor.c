...
#if defined(CONFIG_SENSORS_SSP_PROX_SETTING)
...
{
	struct ssp_data *data = dev_get_drvdata(dev);
	return data->proximity_ops->get_proximity_trim_check(data, buf);
}
#endif
...
#if defined(CONFIG_SENSORS_SSP_PROXIMITY_AUTO_CAL_TMD3725)
static DEVICE_ATTR(prox_trim_check, S_IRUGO, proximity_trim_check_show, NULL);
#endif
