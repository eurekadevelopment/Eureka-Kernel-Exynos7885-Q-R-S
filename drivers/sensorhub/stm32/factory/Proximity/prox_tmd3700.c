...
u16 get_proximity_tmd3700_raw_data(struct ssp_data *data)
{
	u16 uRowdata = 0;
	char chTempbuf[4] = { 0 };
...
ssize_t get_proximity_tmd3700_name(char *buf)
{
	return sprintf(buf, "%s\n", "TMD3700");
}

ssize_t get_proximity_tmd3700_vendor(char *buf)
{
	return sprintf(buf, "%s\n", "AMS");
}

ssize_t get_proximity_tmd3700_avg_raw_data(struct ssp_data *data, char *buf)
{
...
*/
ssize_t set_proximity_tmd3700_avg_raw_data(struct ssp_data *data, const char *buf)
{
	char chTempbuf[4] = { 0 };
	int ret;
	int64_t dEnable;
...
int proximity_store_cancelation(struct ssp_data *data, int iCalCMD)
{
	...
	if (iCalCMD) {
		data->uCrosstalk = get_proximity_tmd3700_raw_data(data);
	...
	ret = vfs_write(cancel_filp, (u8 *)&data->uProxCanc,
...
ssize_t get_proximity_tmd3700_cancel(struct ssp_data *data, char *buf)
{
...
ssize_t set_proximity_tmd3700_cancel(struct ssp_data *data, const char *buf)
{
	int iCalCMD = 0, ret = 0;
...
ssize_t get_tmd3700_proximity_cancel_pass(struct ssp_data *data, char *buf)
{
	pr_info("[SSP] %s, %u\n", __func__, data->uProxCalResult);
	return snprintf(buf, PAGE_SIZE, "%u\n", data->uProxCalResult);
}

ssize_t get_tmd3700_threshold_high(struct ssp_data *data, char *buf)
{
...
ssize_t set_tmd3700_threshold_high(struct ssp_data *data, const char *buf)
{
	u16 uNewThresh;
	int ret = 0;
...
ssize_t get_tmd3700_threshold_low(struct ssp_data *data, char *buf)
{
	ssp_info("uProxThresh = hi - %u, lo - %u",
		data->uProxHiThresh, data->uProxLoThresh);
...
ssize_t set_tmd3700_threshold_low(struct ssp_data *data, const char *buf)
{
	u16 uNewThresh;
	int ret = 0;
...
ssize_t get_proximity_tmd3700_trim_value(struct ssp_data *data, char *buf)
{
	int reet, iReties = 0;
	struct ssp_msg *msg;
	u8 buffer[8] = {0, };
...
struct proximity_sensor_operations prox_tmd3700_ops = {
	.get_proximity_name = get_proximity_tmd3700_name,
	.get_proximity_vendor = get_proximity_tmd3700_vendor,
	.get_proximity_avg_raw_data = get_proximity_tmd3700_avg_raw_data,
	.set_proximity_avg_raw_data = set_proximity_tmd3700_avg_raw_data,
	.get_proximity_cancel = get_proximity_tmd3700_cancel,
	.set_proximity_cancel = set_proximity_tmd3700_cancel,
	.get_threshold_high = get_tmd3700_threshold_high,
	.set_threshold_high = set_tmd3700_threshold_high,
	.get_threshold_low = get_tmd3700_threshold_low,
	.set_threshold_low = set_tmd3700_threshold_low,
	.get_proximity_raw_data = get_proximity_tmd3700_raw_data,
	.get_proximity_cancel_pass = get_tmd3700_proximity_cancel_pass,
	.get_proximity_trim_value = get_proximity_tmd3700_trim_value,
};

void proximity_tmd3700_function_pointer_initialize(struct ssp_data *data)
{
	data->proximity_ops = &prox_tmd3700_ops;
}
