...
u16 get_proximity_tmd3725_raw_data(struct ssp_data *data)
{
	u16 uRowdata = 0;
	char chTempbuf[4] = { 0 };
...
ssize_t get_proximity_tmd3725_name(char *buf)
{
	return sprintf(buf, "%s\n", "TMD3725");
}

ssize_t get_proximity_tmd3725_vendor(char *buf)
{
	return sprintf(buf, "%s\n", "AMS");
}
...
int proximity_store_cancelation(struct ssp_data *data, int iCalCMD)
{
	...
	if (iCalCMD) {
		data->uCrosstalk = get_proximity_tmd3725_raw_data(data);
	...
	iRet = vfs_write(cancel_filp, (u8 *)&data->uProxCanc,
...
ssize_t get_proximity_tmd3725_cancel(struct ssp_data *data, char *buf)
{
...
ssize_t set_proximity_tmd3725_cancel(struct ssp_data *data, const char *buf)
{
	int iCalCMD = 0, ret = 0;
...
ssize_t get_proximity_tmd3725_cancel_pass(struct ssp_data *data, char *buf)
{
...
struct proximity_sensor_operations prox_tmd3725_ops = {
	.get_proximity_name = get_proximity_tmd3725_name,
	.get_proximity_vendor = get_proximity_tmd3725_vendor,
	...
	.get_proximity_cancel = get_proximity_tmd3725_cancel,
	.set_proximity_cancel = set_proximity_tmd3725_cancel,
	.get_proximity_cancel_pass = get_proximity_tmd3725_cancel_pass,
	...
	.get_proximity_raw_data = get_proximity_tmd3725_raw_data,
	.get_proximity_trim_value = get_proximity_tmd3725_trim_value,
};

void proximity_tmd3725_function_pointer_initialize(struct ssp_data *data)
{
	data->proximity_ops = &prox_tmd3725_ops;
}
