...
static inline void sc_hwset_clk_req(struct sc_dev *sc, unsigned int rq)
{
	/* Auto clock gating for Q-channel
	 * - 1 : disable
	 * - 0 : enable
	 * if rq is 0 and no transaction,
	 * q-channel block is power off automatically.
	 */
	__raw_writel(rq, sc->regs + SCALER_CLK_REQ);
}
